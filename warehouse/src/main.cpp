...
float readWeightGrams() {
  ...
  long raw = loadcell.get_units(3);

  // Wokwi HX711 type "5kg": raw 0-2100 tuong ung 0-5000g
  // Phan cung that: hieu chinh calScale bang lenh CAL
  float w = raw * calScale;

  if (w > 5500.0f && !calibrated) {
    calScale = 5000.0f / raw;
    calibrated = true;
    Serial.printf("[CAL] Scale adjusted to %.4f (raw=%ld -> 5000g)\n", calScale, raw);
    w = 5000.0f;
  }
